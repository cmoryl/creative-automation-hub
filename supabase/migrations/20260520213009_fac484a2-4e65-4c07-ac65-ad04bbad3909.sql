...
-- Update seed function to include the TransPerfect Case Study template
...
BEGIN
  IF EXISTS (
    ...
    -- Still ensure TransPerfect template exists for already-seeded workspaces
    IF NOT EXISTS (
      SELECT 1 FROM public.templates
      ...
      INSERT INTO public.templates (workspace_id, engine, name, source_ref, preview_url, variables) VALUES
        (_workspace_id, 'illustrator', '[Live] TransPerfect Case Study – A4',
...
-- Backfill: add the TransPerfect template into every existing workspace that doesn't already have it
INSERT INTO public.templates (workspace_id, engine, name, source_ref, preview_url, variables)
SELECT w.id, 'illustrator', '[Live] TransPerfect Case Study – A4',
  'bridge://templates/TP_CaseStudy_MASTER_v001.ai',
  '/templates/transperfect-case-study-hero.jpg',
  '[
    {"name":"case_study_title","type":"text","label":"Case Study Title","placeholder":"Organic & Paid Campaign Management for Qantas"},
...
WHERE NOT EXISTS (
  ...
  WHERE t.workspace_id = w.id AND t.name = '[Live] TransPerfect Case Study – A4'
);
